...
const PaginationBar = ({ data, page, setPage }) => {
  ...
  return (
    <Pagination>
      <Pagination.First onClick={handleFirst} />
      <Pagination.Prev disabled={page === 1} onClick={handlePrev} />
      <Pagination.Item hidden={page <= 3} onClick={() => handleVal(1)}>
        {1}
      </Pagination.Item>
      <Pagination.Ellipsis hidden={page <= 3} />

      <Pagination.Item hidden={page <= 2} onClick={() => handleVal(page - 2)}>
        {page - 2}{" "}
      </Pagination.Item>
      <Pagination.Item hidden={page === 1} onClick={() => handleVal(page - 1)}>
        {page - 1}
      </Pagination.Item>
      <Pagination.Item active>{page}</Pagination.Item>
      <Pagination.Item
        hidden={page >= totalPage - 1}
        onClick={() => handleVal(page + 1)}
      >
        {page + 1}
      </Pagination.Item>
      <Pagination.Item
        hidden={page >= totalPage - 2}
        onClick={() => handleVal(page + 2)}
      >
        {page + 2}
      </Pagination.Item>
